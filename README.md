...
В репозитории лежит готовый скилл — `skills/git-xfer/SKILL.md`. Инструкция
в нём самодостаточная: агенту достаточно дать ссылку на этот репозиторий,
дальше он сам проверит, установлен ли инструмент, поставит при
необходимости, заведёт конфиг, спросив пути и ветки, и только потом
возьмётся за перенос. Он делает `doctor`/`sync`/`list`, разбирает пометки,
предлагает набор коммитов с обоснованием, показывает сухой прогон и
применяет после вашего «да». Конфликты разбирает по политике из конфига,
а частые поломки опознаёт по таблице симптомов.
