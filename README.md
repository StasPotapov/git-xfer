...
Дайте агенту ссылку на этот репозиторий и скажите, что нужно:

> `https://github.com/StasPotapov/git-xfer` — перенеси коммит `a1b2c3d`
> из `~/dev/repo-a` в `~/dev/repo-b`

Дальше он всё сделает сам. В репозитории лежит скилл
`skills/git-xfer/SKILL.md` — самодостаточная инструкция: агент по ней
поставит инструмент, спросит пути и ветки, заведёт конфиг, проверит
готовность, покажет, что уже переносили и где будут конфликты,
и применит после вашего «да».

Потом достаточно кидать ему хеш или заголовок коммита — «перенеси вот
этот в личный репозиторий».

Скилл написан для Claude Code — если положить его в `~/.claude/skills`,
появится команда `/git-xfer`. Но `SKILL.md` это обычный текст: любому
агенту, умеющему запускать команды в терминале, хватит ссылки.
