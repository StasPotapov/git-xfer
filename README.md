...
Вникать в подкоманды необязательно. В репозитории лежит скилл
`skills/git-xfer/SKILL.md` — инструкция для ИИ-агента, который сделает
всё сам. Клонируйте (скилл живёт в репозитории) и подложите его агенту —
сам инструмент он поставит себе дальше сам:
...
Дальше `/git-xfer` или просто словами: «настрой перенос между вот этими
двумя репозиториями». Агент проверит, установлен ли инструмент, поставит,
спросит пути и ветки, заведёт конфиг и прогонит проверку готовности.

После настройки это выглядит так: кидаете агенту хеш или заголовок
коммита — «перенеси вот этот в личный репозиторий» — и дальше он всё
делает сам: подтягивает объекты, проверяет, не переносили ли этот коммит
раньше, показывает сухой прогон с ожидаемыми конфликтами, спрашивает
подтверждение и применяет. Конфликт разбирает по политике, которую вы
задали в конфиге ([ниже](#скилл-для-claude-code)).

Скилл написан для Claude Code, но `SKILL.md` — обычный текст: любому
агенту, который умеет запускать команды в терминале, достаточно дать
на него ссылку.

### Почему не просто «нейронка, перенеси коммиты»

Потому что агенту с голым `git cherry-pick` не на что опереться. Он не
помнит, что переносил в прошлый раз, — и молча продублирует коммит или
пропустит нужный. Не отличит «конфликт» от «коммит стал пустым, потому
что это изменение уже там». Упрётся в конфликт на третьем коммите
из десяти и оставит репозиторий в середине незавершённой операции.
Проверять за ним придётся вам — то есть ровно то, от чего вы хотели
избавиться.

`git-xfer` не даёт агенту места для импровизации: что уже перенесено,
считает инструмент — четырьмя каналами сразу, а не на глаз; конфликты
показываются до того, как что-то изменилось в рабочем дереве; серия
переживает конфликт в середине и доводится до конца; каждый запущенный
git-вызов ложится в журнал. Агент здесь решает, *что* переносить,
и разбирает конфликты — а считает, проверяет и применяет инструмент.
...
`apps/mobile/icons/logo.svg` против просто `icons/logo.svg`. Перенос
«как есть» создал бы в личном репозитории лишнюю вложенность
`apps/mobile/…`.

Подкаталог объявляется ключом `<сторона>_prefix`:
...
- **`--to b` срезает префикс, `--to a` его надевает.** Направление решает,
  какая сторона источник, и префиксы применяются соответственно.
- **Коммиты, не задевшие подкаталог, в список не попадают.** Значит
  `--limit` теперь считает коммиты проекта, а не коммиты монорепозитория —
...
`ios/` и `tools/` монорепозитория для merge выглядят как «добавлено нами»
и остаются на месте.
...
Рядом со статусом бывает вторая пометка — она про охват, а не про
дедупликацию, и сочетается с любым из трёх значков:
...
```bash
sh tests/smoke.sh          # 206 проверок
